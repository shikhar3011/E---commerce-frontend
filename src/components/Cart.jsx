...
const Cart = () => {
    const { cart, decreaseQty, addToCart, removeFromCart,clearCart } = useContext(Appcontext); // Ensure addToCart is destructured
    ...
    return (
        ...
        {cart?.items?.length==0 ? (
            <>
            <div className='text-center my-5'><button className='btn btn-warning mx-3' style={{ fontWeight: 'bold', fontSize: '1.2rem' }}
        onClick={()=>navigate('/')}>Continue Shopping</button></div>
            </>
        ):(
                ...
                <div key={product._id} className='container bg-dark my-5 p-3 text-center'>
                    <div style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center' }}>
                        ...
                        <div className="cart_action">
                            <button className="btn btn-warning mx-3" style={{ fontWeight: 'bold' }} onClick={() => decreaseQty(product?.productId, 1)}>Qty--</button>
